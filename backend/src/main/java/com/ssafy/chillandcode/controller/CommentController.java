...
public class CommentController {
    ...
    public ResponseEntity<?> createComment(
        ...
        if (req.getContent() == null || req.getContent().trim().isEmpty()) {
            return ResponseEntity.badRequest()
                    .body(new MessageResponse(false, "댓글 내용은 비어 있을 수 없습니다."));
        }
        ...
        if (commentId == null) {
            return ResponseEntity.status(404)
                    .body(new MessageResponse(false, "해당 게시글을 찾을 수 없습니다."));
        }

        return ResponseEntity.ok(
                new CommentCreateResponse(commentId, true, "댓글이 성공적으로 등록되었습니다."));
    }
    ...
    public ResponseEntity<?> updateComment(
        ...
        if (!success) {
            return ResponseEntity.status(403)
                    .body(new MessageResponse(false, "댓글 작성자만 수정할 수 있습니다."));
        }

        return ResponseEntity.ok(
                new MessageResponse(true, "댓글이 성공적으로 수정되었습니다."));
    }
    ...
    public ResponseEntity<?> deleteComment(
        ...
        if (!success) {
            return ResponseEntity.status(403)
                    .body(new MessageResponse(false, "댓글 작성자만 삭제할 수 있습니다."));
        }

        return ResponseEntity.ok(
                new MessageResponse(true, "댓글이 삭제되었습니다."));
    }
